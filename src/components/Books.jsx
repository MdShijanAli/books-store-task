...
export default function Books({ searchData }) {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        setIsLoading(true)
        const response = await axios.get(`https://gutendex.com/books/?page=${ page }${ searchData ? `&search=${ searchData }` : '' }${ topic ? `&topic=${ topic }` : '' }`);
  ...
  return (
    <div>
      <div className="flex justify-end mb-3">
        <select name="" id="" className="border px-5 py-2" onChange={(e) => setTopic(e.target.value)}>
